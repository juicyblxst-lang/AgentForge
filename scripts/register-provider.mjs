...
if (!agentHost) throw new Error("ERC8183_AGENT_URL must be the public provider URL, e.g. https://provider.example");
...
const executionEndpoint = `${agentHost}/erc8183`;
...
const agentUri = sdk.generateAgentUri({
  name,
  description,
  endpoints: [new AgentEndpoint({ name: "ERC-8183", endpoint: executionEndpoint, version: "0.1.0" })],
});

console.log(`Registering ${name} from wallet ${sdk.walletAddress}`);
console.log(`ERC-8183 execution endpoint: ${executionEndpoint}`);
