...
function App() {
  ...
  async function execute(){
    ...
    try{
      setBusy(true);setStatus(`Creating ERC-8183 job for ${selected.name} through AgentForge provider…`);
      const executionProvider=providerAddress as `0x${string}`;
      const value=await createAndFundJob(wallet.account,wallet.wallet,executionProvider,DEMO_BUDGET,`Hire ${selected.name} (ERC-8004 agent ${selected.agentId}): ${selected.description||'BNB Chain agent execution'}`);
      ...
      await waitForJobStatus(value.jobId,'SUBMITTED',300_000,(chainStatus)=>{if(chainStatus==='FUNDED')setStatus(`Job #${value.jobId} funded. AgentForge provider is executing ${selected.name}…`)});
      ...
      const job=await getJob(value.jobId);const verified=createVerification.verified&&fundVerification.verified&&job.client.toLowerCase()===wallet.account.toLowerCase()&&job.provider.toLowerCase()===executionProvider.toLowerCase()&&job.status===2;
  ...
  const normalizedProvider=providerAddress?.toLowerCase()||null;
  const normalizedSelectedWallet=selectedWallet.toLowerCase();
  const isAgentForgeProvider=Boolean(normalizedProvider);
  const canExecute=Boolean(identity&&capability?.verified&&providerAddress&&selectedWallet&&isAgentForgeProvider&&wallet);
