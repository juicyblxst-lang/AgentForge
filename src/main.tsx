...
function App() {
  ...
  const isAgentForgeProvider=Boolean(normalizedProvider&&normalizedSelectedWallet&&normalizedProvider===normalizedSelectedWallet);
  const canExecute=Boolean(identity&&capability?.verified&&providerAddress&&selectedWallet&&isAgentForgeProvider);
  ...
  return <main className="shell"><header className="nav"><div className="brand">AGENTFORGE</div><div className="network"><span/> BSC Testnet · 97</div><button className="wallet" onClick={connect}>{wallet?`${wallet.account.slice(0,6)}…${wallet.account.slice(-4)}`:'Connect wallet'}</button></header>
    ...
    <section className="market"><div className="toolbar"><div className="categories">{categories.map(item=><button key={item} className={category===item?'active':''} onClick={()=>setCategory(item)}>{item}</button>)}</div><button className="refresh" onClick={()=>void load()}>Refresh</button></div><p className="status">{status}</p>
      {selected?<section className="detail"><button className="back" onClick={()=>setSelected(null)}>← Marketplace</button><div className="detail-head"><div><p className="eyebrow">AGENT DETAIL</p><h2>{selected.name}</h2><p>{selected.description||'ERC-8004 registered agent on BSC Testnet.'}</p></div><span className="verified">{capability?.verified?'✓ Identity + capability verified':identity?'✓ Identity verified':'Verifying…'}</span></div>
        ...
        <div className="permission-panel"><p className="eyebrow">AUTHORIZATION</p><h2>Review execution</h2><p>{isAgentForgeProvider?'This agent is the AgentForge ERC-8183 provider. You can authorize a real BSC Testnet execution.':'This agent is verified and discoverable, but it is not currently the AgentForge ERC-8183 execution provider. No transaction will be attempted for this agent.'}</p><div className="facts"><div><small>NETWORK</small><strong>BSC Testnet (97)</strong></div><div><small>PROTOCOL</small><strong>ERC-8183</strong></div><div><small>MAX PAYMENT</small><strong>0.01 U</strong></div></div>{isAgentForgeProvider&&<p className="provider-state">✓ AgentForge provider available · ERC-8183 execution enabled</p>}{!isAgentForgeProvider&&providerAddress&&selectedWallet&&<p className="provider-state">Marketplace agent only · AgentForge provider is {providerAddress.slice(0,6)}…{providerAddress.slice(-4)}</p>}<button className="authorize" disabled={busy||!canExecute} onClick={()=>void execute()}>{busy?'Executing…':!providerAddress?'Provider not configured':!capability?.verified?'Capability verification required':!selectedWallet?'Agent wallet unavailable':!isAgentForgeProvider?'Agent verified · not an AgentForge execution provider':wallet?'Authorize & execute':'Connect wallet to execute'}</button></div>
