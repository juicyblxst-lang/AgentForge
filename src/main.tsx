...
function App() {
  ...
  const load=React.useCallback(async()=>{try{setStatus('Discovering real BSC Testnet agents…');setAgents(await discoverBscAgents());setStatus('Live Agent0 discovery')}catch(e){setStatus(e instanceof Error?e.message:'Discovery failed')}},[]);
  React.useEffect(()=>{void load();setHistory(listStoredExecutions())},[load]);
  async function openAgent(agent:MarketplaceAgent){setSelected(agent);setResult(null);setIdentity(null);try{setIdentity(await getAgentIdentity(BigInt(agent.agentId)))}catch(e){setStatus(e instanceof Error?e.message:'Identity verification failed')}}
  async function connect(){try{setWallet(await connectWallet())}catch(e){setStatus(e instanceof Error?e.message:'Wallet connection failed')}}
  async function execute(){if(!selected)return;if(!wallet){await connect();return}if(!identity)return setStatus('Verify the agent identity first.');if(!selected.agentWallet)return setStatus('Agent has no ERC-8004 agent wallet; execution blocked.');setBusy(true);setStatus('Creating and funding ERC-8183 job…');try{const value=await createAndFundJob(wallet.account,wallet.wallet,selected.agentWallet as `0x${string}`,10_000_000_000_000_000n,'AgentForge test execution');setResult(value);const stored:StoredExecution={id:`${value.jobId}-${value.fundHash}`,agentId:selected.agentId,agentName:selected.name,wallet:wallet.account,chainId:97,protocol:'ERC-8183',jobId:String(value.jobId),createHash:value.createHash,fundHash:value.fundHash,status:'FUNDED',createdAt:new Date().toISOString()};saveStoredExecution(stored);setHistory(listStoredExecutions());setStatus(`Job ${value.jobId} funded on BSC Testnet`)}catch(e){setStatus(e instanceof Error?e.message:'Execution failed')}finally{setBusy(false)}}
  ...
  return <main className="shell"><header className="nav"><div className="brand">AGENTFORGE</div><div className="network"><span/> BSC Testnet · 97</div><button className="wallet" onClick={connect}>{wallet?`${wallet.account.slice(0,6)}…${wallet.account.slice(-4)}`:'Connect wallet'}</button></header>
    ...
    <section className="market"><div className="toolbar"><div className="categories">{categories.map(item=><button key={item} className={category===item?'active':''} onClick={()=>setCategory(item)}>{item}</button>)}</div><button className="refresh" onClick={()=>void load()}>Refresh</button></div><p className="status">{status}</p>
      ...
      </section>:<><div className="agents">{visible.length?visible.map(agent=><AgentCard key={agent.id} agent={agent} onOpen={()=>void openAgent(agent)}/>):<div className="empty-state"><div className="orb">A</div><p className="eyebrow">{status}</p><h2>No agents loaded</h2><p>Set the Agent0 Graph API key in your local environment, then refresh.</p></div>}</div>{history.length>0&&<div className="history"><p className="eyebrow">EXECUTION HISTORY · SURVIVES REFRESH</p>{history.slice(0,5).map(x=><div className="history-row" key={x.id}><strong>#{x.jobId} · {x.agentName}</strong><span>{x.status}</span><code>{x.fundHash.slice(0,12)}…</code></div>)}</div>}</>}
    </section></main>
}
